...
.
├── data/
│ └── test_data.py # Тестовые данные (учётки, ошибки)
├── helpers/
│ └── helpers.py # Вспомогательные функции (login, ожидания)
├── locators/
...
> В requirements.txt должны быть: selenium, pytest, pytest-html, pytest-cov.
...
Конструктор	BUNS_TAB, SAUCES_TAB, FILLINGS_TAB, CONSTRUCTOR_TABS
Регистрация / Вход	REGISTER_NAME, REGISTER_EMAIL, REGISTER_PASSWORD, REGISTER_SUBMIT, REGISTER_ERROR, LOGIN_EMAIL, LOGIN_PASSWORD, LOGIN_SUBMIT, LOGIN_LINK, REGISTER_LINK, RESTORE_LINK, LINK_LOGIN
...
Файл `helpers.py` содержит полезные функции:
